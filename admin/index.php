...
<div class="page-content" id="app">
    <div class="top-note page-topnav">
        <p>Mass upload items to your marketplace using the special csv. <a href="#" id="formatlink">Get the format
                here.</a></p>
    </div>
    ...
    <div class="page-topnav secondnary-topnav">
            ...
            <div class="d-flex">
                <div class="browse-element">
                    <div class="form-group">
                        <input type="file" name="file-7[]" id="file-7" accept=".csv" class="inputfile"
                            data-multiple-caption="{count} Upload File" multiple @change="loadCSV($event)">
                        <label for="file-7"><span class="archive-name">Upload File</span><span class="btn-inputfile">
                                Browse</span></label><span class="btn-inputfileclear">Clear</span><span
                            class="result-found" style="color:green">{{ count }} Item/s Found.</span><span
                            v-if="failedcount != 1" class="result-found" style="color:red">{{failedcount - 1}} Warning/s
                            (empty Fields).</span>
                        <span v-if="invalidPriceCount != 0" class="result-found" style="color:red">{{invalidPriceCount}}
                            Invalid Price format.</span>
                            <span v-if="invalidCategoryCount != 0" class="result-found" style="color:red">{{invalidCategoryCount}}
        ...
        <div class="upload-section mt-30">
            <button v-on:click=onUpload
                :class=" count == 0  || success_all != 0 || failed_all != 0  ? { disable : true} : {active: true} ">Upload</button>
        ...
        <div class="mass-upload-browser">
            ...
            <div class="d-flex">
                ...
                <div class="status-container">
                    ...
                    <a href="#" id="download_success" :class=" success_all == 0 ? { hasSuccess : true } : '' ">Download
                        successful imports</a>

                    <span class="failed">Failed: {{ failed_all }}</span>

                    <a href="#" id="download_failed" :class=" failed_all == 0 ? { hasFailed : true } : '' ">Download
                        failed imports</a>
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="(item,itemkey) in upload_error">
                            ...
                            <td class='resultSuccess' :class=" item.code == 'Failed' ? { resultFailed : true } : '' ">
                                {{ item.code }} </td>
                            <td> {{ item.error }} </td>
                        </tr>
            ...
            <div class="undo-section mt-30">
                ...
                <span class="success" :class="revert_total == 0 ? { hasFailed : true } : '' ">Total items:
                    {{ revert_total }} </span>
            </div>
...
$(document).ready(function() {
    $("body").on("click", ".upload-section.active button", function() {
        ...
        $(".mass-upload-browser").find(".table-responsive").css({
            overflow: "hidden"
        });
    });
    setTimeout(function() {
    ...
    $("body").on("click", ".btn-inputfileclear", function() {
        ...
        $(".csv-extractor .table tbody").html(
